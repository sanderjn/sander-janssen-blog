<template>
  <main>
    ...
    <div class="mt-10 space-y-16">
      <article
        v-for="post in posts"
        v-if="post.attributes.status === 'published'"
        :key="post.attributes.title"
        :id="post.path.replace('/blog/', '')"
        class="
          relative
          flex flex-col
          justify-end
      ...
      >
        ...
        <div
          ...
          "
        ></div>
        <span class="text-xs font-mono text-white opacity-75"
          >{{ prettyDate(post) }} in {{ post.attributes.category }}</span
        >
        <nuxt-link
          class="my-0 text-lg text-white font-display z-10 md:text-2xl"
          :to="post.path"
        >
    ...
    return {
      ...
      meta: [
        // Each object in this array is its own meta tag
        {
          ...
          name: "description",
          content:
            "Blog van Sander Janssen. Ik schrijf soms stukjes over technologie, mijn leven en andere zaken die me bezighouden.",
        },
      ],
      bodyAttrs: {
        class: "blog",
